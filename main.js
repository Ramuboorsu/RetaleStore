...
$("body").delegate(".category","click",function(event){
		$("#get_branded").html("<h3>Loading123...</h3>");
		event.preventDefault();
		var did = $(this).attr('cid');
			...
			url		:	"action.php",
			method	:	"POST",
			data	:	{get_branded:1,did:did},
			success	:	function(data){
				$("#get_branded").html(data);
				if($("body").width() < 480){
					$("body").scrollTop(683);
				}
			}
...
$("body").delegate(".brandname","click",function(event){
		$("#get_product").html("<h3>Loading123...</h3>");
		event.preventDefault();
		var brid = $(this).attr('brndid');
		
			$.ajax({
			url		:	"action.php",
			method	:	"POST",
			data	:	{get_brandname:1,brid:brid},
			success	:	function(data){
				$("#get_product").html(data);
				if($("body").width() < 480){
					$("body").scrollTop(683);
  ...
  $('.ui-slider-handle').on('click',function(){
	  ...
	  var max_price=$( "#slider-range" ).slider( "values", 1 );
		...
		$.ajax({
			url:'action.php',
			type:'POST',
			data:{brandnamerange:1,min_price:min_price,max_price:max_price,brandid:brandid},
			success	:	function(data){
				$("#get_product").html(data);
				if($("body").width() < 480){
					$("body").scrollTop(683);
  ...
  $('.ui-slider-handle').on('click',function(){
	  ...
	  var max_price=$( "#slider-range" ).slider( "values", 1 );
		...
		$.ajax({
			url:'action.php',
			type:'POST',
			data:{range:1,min_price:min_price,max_price:max_price,rid:rid},
			success	:	function(data){
				$("#get_product").html(data);
				if($("body").width() < 480){
					$("body").scrollTop(683);
  ...
  $('.ui-slider-handle').on('click',function(){
	  ...
	  var max_price=$( "#slider-range" ).slider( "values", 1 );
		...
		$.ajax({
			url:'action.php',
			type:'POST',
			data:{outrange:1,min_price:min_price,max_price:max_price},
			success	:	function(data){
				$("#get_product").html(data);
				if($("body").width() < 480){
					$("body").scrollTop(683);
		...
		$.ajax({
			...
			success	:function(data){
				if(data == "login_success"){
					window.location.href = "yourdemo.php";
				}else if(data == "cart_login"){
					window.location.href = "cart123.php";
				}else{
					$("#e_msg").html(data);
					$(".overlay").hide();
				}
